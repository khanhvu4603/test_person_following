...
# ------------------------------------------------------------
# Webcam demo: ADMIN-only label, best-match + temporal memory
# FACE: InsightFace (RetinaFace alignment + ArcFace) + cosine
# BODY: Person = MobileNet-SSD (OpenCV DNN), Body Embedding = MobileNetV2(GAP) + PCA/KNN
# ------------------------------------------------------------
...
def body_arr(frame, box):
    x1,y1,x2,y2 = expand(box, frame, 0.20) if isinstance(frame, tuple) else expand(box, frame.shape, 0.20)
...
def predict_body_conf(emb, pca, knn, mean, centroid=None):
...
def main():
    ...
    ap.add_argument("--accept_face", type=float, default=0.35)   # cosine
    ap.add_argument("--accept_body", type=float, default=0.35)
    ...
    if args.enroll:
        print("[ENROLL] Collecting FACE(ArcFace) + BODY embeddings for", args.label)
        E = []; B = []
        while True:
            ok,frame = cap.read()
            if not ok: break
            ...
            cv2.putText(frame, f"ENROLL: face {len(E)} | body {len(B)}  (q to finish)", (10,30),
                        cv2.FONT_HERSHEY_SIMPLEX,0.7,(255,255,255),2)
            cv2.imshow("ENROLL (ArcFace + Body)", frame)
    ...
    print("[RUN] ArcFace + cosine, ADMIN by best-match + temporal memory. q=quit.")
    ...
    while True:
        ...
        if body_models is not None:
            ...
            for pb in pboxes:
                ...
                if arrb is None:
                    ...
                    from tensorflow.keras.models import Model  # lazy import safe
                    bemb = mb2.predict(arrb, verbose=0).flatten()
                    body_confs.append(float(predict_body_conf(bemb, bpca, bknn, bmean, centroid=bcent)))
        else:
        ...
        cv2.putText(frame, f"STATUS: {status} (best face cos {face_best:.2f} | best body {body_best:.2f} | EMA {admin_track['score_ema']:.2f})",
                    (10,35), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (50,220,50) if is_admin else (0,165,255), 2)

        cv2.imshow("ADMIN (ArcFace + cosine) + Temporal Memory", frame)
        if cv2.waitKey(1)&0xFF==ord('q'): break
